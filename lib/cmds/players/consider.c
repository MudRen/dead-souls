...
mixed cmd(string str) {
    ...
    else if(totalscore > 80) write("没问题。");
    else if(totalscore > 60) write("非常简单。");
    else if(totalscore > 40) write("相当容易。");
    else if(totalscore > 20) write("你应该能应付。");
    else if(totalscore > 0) write("势均力敌。");
    else if(totalscore > -20) write("一场硬仗。");
    else if(totalscore > -40) write("一场恶战。");
    else if(totalscore > -60) write("巨大的挑战。");
    else if(totalscore > -80) write("不明智的选择。");
    else if(totalscore > -100) write("真是个坏主意。");
    else write("自杀行为。");
    return 1;
}

string GetHelp() {
    return ("用法: consider <生物>\n\n"
            "让你了解击败某个生物的难度。"
            "评估相对力量、技能、武器、护甲、等级、生命值等。"
            "评估结果可能不太准确，因为战斗中的变化"
            "（如对手更换武器）或具有特殊防护等级的物品"
            "（如魔法护甲）可能未被考虑在内。");
}
