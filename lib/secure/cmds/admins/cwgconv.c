...
string GetHelp() {
    message("help", "语法：cwgconv <源路径> <区域名> <域名>\n\n"
            "尝试将 CWG 区域转换为 Dead Souls 域。\n"
            "示例：cconv /tmp/areadir 3 newdomain\n"
            "这将尝试创建 /domains/newdomain 并将\n"
            "/tmp/areadir/3 中描述的区域转换为新 DS 域中的 LPC 文件。"
            "\n", this_player());
}
