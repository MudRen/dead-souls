...
protected void create() {
    ...
    SetErrorMessage("看什么或看里面？");
    SetHelp("用法：look\n"
            "      look at <物品>\n"
            "      look in <容器>\n"
            "      look at <物品> in <容器>\n"
            "      look at <物品> on <物品>\n\n"
            "不带参数时，此命令允许你查看周围区域的描述，包括与你同在的其他事物。\n\n"
            "如果你看某个东西，你会得到你正在看的东西的详细描述。"
            "你应该能够查看使用不带参数的 look 命令时在房间中看到的任何东西。"
            "任何你看不到的东西都被视为 bug。\n\n"
            "另见：peer");  
}
...
mixed do_look() {
    if(environment(this_player()) && !this_player()->GetInvis() &&
        ...
        environment(this_player())->eventPrint(this_player()->GetName() +
                "环顾四周。", this_player(), (MSG_ENV|MSG_ANNOYING));
